...
export default function UpdateListValue() {
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await apiServices.get(
        `/list/value-sub-list?parentId=${params.id}&name=${debouncedSearch}&page=${page}&limit=${rows}}`,
      );
      const result = response.data;
      if (result.success) {
             settitle(response.data.parent);
      ...
      if (response?.data?.success) {
        ...
        customers((prevData) =>
          prevData.map((category) =>
            category._id === editItemId ? { ...category, name: editName } : category
          )
        );
      } else {
  ...
  return (
    <div className="card">
      <GoPrevious route={'/system-setup/list-value'} />
      <Toast ref={toast} />
            <Dialog header="Update Category" visible={visible} style={{ width: '50vw' }} onHide={() => {if (!visible) return; setVisible(false); }}>
            <label htmlFor="">Name</label>
                   ...
                   <div className="">
                   <Button loading={updateLoder}  onClick={updateCategory} severity="warning">
                      Update
                    </Button>
                   </div>
                    </div>
      ...
      <div className=" mb-5 flex justify-between  pb-4">
        <div className="flex items-center justify-start gap-2">
          ...
          List of Value: <span className="font-semibold">{title}</span>
          </label>
          <Badge
