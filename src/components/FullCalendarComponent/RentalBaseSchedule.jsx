...
// //   { id: "auditorium-c", name: "Auditorium C", capacity: 120, type: "Auditorium" },
// //   { id: "auditorium-d", name: "Auditorium D", capacity: 100, type: "Auditorium" },
// //   { id: "auditorium-e", name: "Auditorium E", capacity: 180, type: "Auditorium" },
// //   { id: "auditorium-f", name: "Auditorium F", capacity: 160, type: "Auditorium" },

// // ];
// // const initialEvents = [
// //   // March 6 events
// //   { id: "1", title: "Meeting", start: "2025-03-06", end: "2025-03-13", resourceId: "auditorium-c", type: "meeting" },
// //   { id: "2", title: "Event 1", start: "2025-03-06", end: "2025-03-06", resourceId: "auditorium-d", type: "event" },
// //   { id: "3", title: "Training", start: "2025-03-06", end: "2025-03-06", resourceId: "auditorium-e", type: "training" },
// //   { id: "4", title: "Workshop", start: "2025-03-06", end: "2025-03-06", resourceId: "auditorium-f", type: "workshop" },

// // ];

// // Helper function to generate dates for the timeline based on view type
// const generateTimelineDates = (currentDate, viewType) => {
//   switch (viewType) {
//     case VIEW_TYPES.DAY:
//       return [currentDate];
...
// // Helper function to calculate position and width for event blocks
// const calculateEventPosition = (event, dates) => {
//   const startDate = new Date(event.start);
//   const endDate = new Date(event.end);

//   // Find the index of the start and end dates in the timeline
...
//   // If dates are not in our timeline range, adjust accordingly
//   const adjustedStartIndex = startIndex === -1 ? 0 : startIndex;
//   const adjustedEndIndex = endIndex === -1 ? dates.length - 1 : endIndex;

//   // Calculate position and width as percentage of the grid
//   // We need to account for the first column (resource name) in our grid
//   const totalColumns = dates.length;
//   const left = `${(adjustedStartIndex / totalColumns) * 100}%`;
//   const width = `${((adjustedEndIndex - adjustedStartIndex + 1) / totalColumns) * 100}%`;
...
//   // Calculate grid column positions for proper alignment
//   const gridColumnStart = adjustedStartIndex + 4; // +2 because CSS grid is 1-based and we have a resource column
//   const gridColumnEnd = adjustedEndIndex + 5; // +3 because end is exclusive, 1-based indexing, and resource column
//   return (
//     <div
//       className={clsx(
//         "flex items-center rounded px-2 py-1 text-xs text-dark",
//         eventColors[event.type] || "bg-green-500",
//       )}
//       style={{
//         gridColumnStart,
//         gridColumnEnd,
...
// // Helper function to check if an event falls within a date range
// const isEventInDateRange = (event, dates) => {
...
//   return (
//     (eventStart <= rangeEnd && eventEnd >= rangeStart) || // Event spans the range
//     (eventStart >= rangeStart && eventStart <= rangeEnd) || // Event starts in the range
//     (eventEnd >= rangeStart && eventEnd <= rangeEnd) // Event ends in the range
//   );
// };

// // Resource Row Component
// const ResourceRow = ({ resource, events, dates }) => {
//   // Filter events by resource and date range
//   const resourceEvents = events.filter(
//     (event) =>
//       event.resourceId === resource.id && isEventInDateRange(event, dates),
//   );

//   return (
//     <div
//       className="grid border-b dark:border-dark-2"
//       style={{
//         gridTemplateColumns: `200px 70px 140px repeat(${dates.length}, 1fr)`,
//       }}
//     >
//       {/* Resource Name */}
//       <div className="border-r p-2 text-sm font-medium dark:border-dark-3">
//         {`${resource.name} ${resource?.orderNo || ""}`}
//       </div>
//       <div className="border-r p-2 text-sm font-medium dark:border-dark-3">
//         <Tag
//           className="font-bold"
...
//           value={resource?.type || ""}
//         /> */}
//         <label className="truncate"> {resource?.type || ""}</label>
//       </div>
...
//           key={index}
//           className={clsx(
//             `h-10 border-r dark:border-dark-2 ${getDay(date) === 0 || getDay(date) === 6 ? "bg-gray-200 dark:bg-dark-3" : "bg-transparent"}`,
//           )}
//         />
//       ))}

//       {/* Events for this resource - positioned with grid */}
//       {resourceEvents.map((event, index) => (
//         <Event key={index} event={event} dates={dates} />
//       ))}
//     </div>
//   );
// };
...
//         <button
//           onClick={handlePrevious}
//           className="rounded bg-orange-600 p-2 hover:bg-orange-700"
//         >
//           <svg
...
//       </div>
//       <div className="">
//         <div className="flex overflow-hidden rounded border dark:border-dark-2">
//           <button
//             onClick={() => setfilterData(Filter_TYPES.ALL)}
//             className={clsx(
//               "px-3 py-1 text-sm font-bold",
//               filterData === Filter_TYPES.ALL
//                 ? "bg-orange-500 text-white"
//                 : "bg-gray-100 hover:bg-gray-200 dark:bg-dark-2",
//             )}
//           >
//             All
//           </button>
//           <button
//             onClick={() => setfilterData(Filter_TYPES.AVAILABLE)}
//             className={clsx(
//               "border-l px-3 py-1 text-sm font-bold dark:border-dark-3",
//               filterData === Filter_TYPES.AVAILABLE
//                 ? "bg-orange-500 text-white"
//                 : "bg-gray-100  hover:bg-gray-200 dark:bg-dark-2",
//             )}
//           >
...
//           </button>
//           <button
//             onClick={() => setfilterData(Filter_TYPES.RENT)}
//             className={clsx(
//               "border-l px-3 py-1 text-sm font-bold dark:border-dark-3 ",
//               filterData === Filter_TYPES.RENT
//                 ? "bg-orange-500 text-white"
//                 : "bg-gray-100  hover:bg-gray-200 dark:bg-dark-2",
//             )}
//           >
...
//           pt={{
//             item: { className: "text-sm p-2" },
//             root: { className: "h-10 text-sm" }, // Adjust height & font size
//             input: { className: "p-1 text-sm font-bold text-[12px]" }, // Smaller padding inside input
//           }}
//           value={category}
...
// // Main Component
// export default function RentalBaseSchedule() {
...
//   const [filterData, setfilterData] = useState(Filter_TYPES.ALL);
...
//   }, [initialEvents]);
//   // Generate timeline dates based on current date and view type
...
//           `${BaseURL}/sub-category?parentId=${category?._id}`,
//         );
...
//           `${BaseURL}/order/rental-timeline?type=${filterData}&&category=${category?._id || ""}&&subCategory=${subCategory?._id||''}`,
//           {
//             headers: {
...
//         if (res.data) {
//           setloading(false);
//           setroomResources(res.data.roomResources);
//           setinitialEvents(res.data.initialEvents);
//         }
//       } catch (error) {
//         setloading(false);
//         console.log(error);
...
//     };
//     fetchdata();
//   }, [filterData, category,subCategory]);

//   useEffect(() => {
//     setTimelineDates(generateTimelineDates(currentDate, viewType));
//   }, [currentDate, viewType]);

//   return (
//     <DndProvider backend={HTML5Backend}>
//       <div className="w-full overflow-x-auto p-4">
//         {/* <h1 className="text-2xl font-bold mb-4">Various Resource-Timeline Views</h1> */}
...
//         {/* Calendar Grid */}
//         <div className="w-full min-w-[1000px] overflow-hidden rounded border dark:border-dark-2">
//           {/* Header Row - Dates */}
//           <div
//             className="grid"
//             style={{
//               gridTemplateColumns: `200px 70px 140px repeat(${timelineDates.length}, 1fr)`,
//             }}
//           >
//             {/* Header label for resource info */}
//             <div className="border-r bg-gray-100 p-2 text-center font-semibold dark:border-dark-2 dark:bg-dark-2">
//               Products Resource
//             </div>
//             <div className="border-r bg-gray-100 p-2 text-center font-semibold dark:border-dark-2 dark:bg-dark-2">
//               Qty
...
//                 key={index}
//                 className={clsx(
//                   "border-r p-1 text-center text-xs font-semibold dark:border-dark-2",
...
//           {loading ? (
//             <>
//               <div className=" items-center flex h-96 justify-center">
//                <div className="">
//                <Loader />
//                </div>
//               </div>
...
//           <div className="text-sm font-bold mb-2">Event Types:</div>
//           <div className="grid grid-cols-5 gap-2">
//             {Object.entries(eventColors).map(([type, color]) => (
//               <div key={type} className="flex items-center">
//                 <div className={`w-4 h-4 mr-2 ${color} rounded`}></div>
