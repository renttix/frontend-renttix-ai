...
export default function RefundModal({ visible, onHide, invoice }) {
  ...
  useEffect(() => {
    ...
    setPercentage(invoice?.orderDiscount || 0);
    setCreditType("Full Invoice Percentage");
    setInternalNotes("");
    setCustomerNotes("");
    setOverrideTotal(0);
    setFlatRefundAmount(0);
    setRefundCategory("");
    setApplyTo("");
    setStep(1);
    setError(null);
  ...
  const discountValue = useMemo(
    ...
    [goodsValue, percentage]
  );
  const newPreTaxSubtotal = useMemo(
    () => Math.max(0, goodsValue - discountValue),
    [goodsValue, discountValue]
  );
  const newTax = useMemo(() => (newPreTaxSubtotal * taxRate) / 100, [newPreTaxSubtotal, taxRate]);
  const newTotal = useMemo(() => newPreTaxSubtotal + newTax, [newPreTaxSubtotal, newTax]);

  const updateAssetCredit = (pIndex, aIndex, changes) => {
    const next = [...updatedProducts];
    const asset = { ...next[pIndex].assetNumbers[aIndex], ...changes };

    // clamp values safely
    const total = Number(asset.total || 0);
    if (asset.creditType === "percent") {
      const v = Number(asset.creditAmount || 0);
      asset.creditAmount = Math.max(0, Math.min(100, v));
    } else if (asset.creditType === "flat") {
      const v = Number(asset.creditAmount || 0);
      asset.creditAmount = Math.max(0, Math.min(total, v));
    }

    next[pIndex] = {
      ...next[pIndex],
      assetNumbers: next[pIndex].assetNumbers.map((a, idx) => (idx === aIndex ? asset : a)),
    };
    setUpdatedProducts(next);
  };

  // ----- VALIDATION -----
  const hasAnyLineCredit = useMemo(() => {
    return (updatedProducts || []).some((p) =>
      (p.assetNumbers || []).some((a) => {
        const ct = a.creditType;
        const ca = Number(a.creditAmount || 0);
        if (!ct) return false;
        if (ct === "percent") return ca > 0 && ca <= 100;
        if (ct === "flat") return ca > 0 && ca <= Number(a.total || 0);
        return false;
      })
    );
  }, [updatedProducts]);
  ...
  const stepIsValid = (s) => {
    ...
    if (s === 1) {
      if (!creditType) msg = "Select a credit type to continue.";
    }

    if (s === 2) {
      if (creditType === "Full Invoice Percentage") {
        if (!(percentage > 0 && percentage <= 100)) {
          msg = "Enter a percentage between 1 and 100.";
        }
      } else if (creditType === "Manual Invoice Override") {
        if (!(overrideTotal >= 0 && overrideTotal < originalTotal)) {
          msg = "New total must be between 0 and less than the original total.";
        }
      } else if (creditType === "Flat Value Refund") {
        if (!(flatRefundAmount > 0 && flatRefundAmount <= originalTotal)) {
          msg = "Refund amount must be greater than 0 and not exceed the original total.";
        }
        if (!refundCategory) msg = "Please select a refund category.";
      } else if (creditType === "Line-Item Credit") {
        if (!hasAnyLineCredit) {
          msg = "Select at least one asset and enter a valid credit amount.";
        }
      }
    }

    if (s === 3) {
      if (!internalNotes?.trim()) msg = "Internal notes are required.";
    }

    if (s === 6) {
      if (!applyTo) msg = "Select how to apply this credit/refund.";
    }
  ...
  const buildRefundPayload = () => {
    ...
    if (creditType === "Manual Invoice Override") {
      payload.breakdown = { overrideTotal };
  ...
  const handleRefundSubmit = async () => {
    // Validate the final choice (applyTo) here too
    ...
    try {
      ...
      await axios.post(`${BaseURL}/invoice/refund`, payload, {
        headers: {
      ...
      // bounce back to review step for fixes
      setStep(4);
    }
  };
useEffect(() => {
  if (visible) {
    ...
    // optional: also reset the form to fresh defaults each open
    setCreditType("Full Invoice Percentage");
    setPercentage(invoice?.orderDiscount || 0);
    setOverrideTotal(0);
    setFlatRefundAmount(0);
  ...
  // ✅ PURE validation for the current step (no setState here)
const currentError = useMemo(() => {
  ...
  if (step === 1) {
    ...
    if (creditType === "Full Invoice Percentage") {
      if (!(percentage > 0 && percentage <= 100)) msg = "Enter a percentage between 1 and 100.";
    } else if (creditType === "Manual Invoice Override") {
      if (!(overrideTotal >= 0 && overrideTotal < originalTotal)) msg = "New total must be between 0 and less than the original total.";
    } else if (creditType === "Flat Value Refund") {
      if (!(flatRefundAmount > 0 && flatRefundAmount <= originalTotal)) msg = "Refund amount must be greater than 0 and not exceed the original total.";
      if (!refundCategory) msg = "Please select a refund category.";
    } else if (creditType === "Line-Item Credit") {
      if (!hasAnyLineCredit) msg = "Select at least one asset and enter a valid credit amount.";
    }
  } else if (step === 3) {
    if (!internalNotes?.trim()) msg = "Internal notes are required.";
  } else if (step === 6) {
  ...
  step, creditType, percentage, overrideTotal, originalTotal,
  flatRefundAmount, refundCategory, hasAnyLineCredit, internalNotes, applyTo
]);
  ...
  const footer = (
    <div className="flex justify-between">
      ...
      {/* Continue is disabled unless current step validates */}
    {step < 6 && step  && step !== 5 && (
  <Button
    label="Continue"
    className="bg-blue-500 border-none"
    onClick={tryNext}
    disabled={!!currentError} // ⬅️ no state updates during render
  />
)}
...
{step === 5 && (
  <Button
    ...
    disabled={submitting || !!currentError} // ⬅️ also use currentError here
  />
)}


      {step === 6 && (
        <Button label="Close" className="bg-green-500 border-none" onClick={onHide} />
      )}
    </div>
  );

 const stepError = (currentError || error) ? (
  ...
  return (
    <Dialog
      ...
      onHide={onHide}
      header={() => stepIconsHeader}
      footer={footer}
    >
      {/* STEPPER */}
      <div className="flex items-center justify-between mb-4 border rounded-xl py-4">
        {[1, 2, 3, 4, 5, 6].map((n) => (
          ...
          <div className="flex gap-2 mb-4 flex-wrap">
            {[10, 20, 25, 50].map((val) => (
              <Button
                key={val}
                label={`${val}%`}
                className={`${percentage === val ? "bg-blue-500 text-white" : "bg-gray-200 text-black"} border-none`}
                onClick={() => {
                  setPercentage(val);
                  setError(null);
                }}
      ...
      {step === 2 && creditType === "Manual Invoice Override" && (
        <div>
          <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 mb-4 flex gap-2">
            ...
            <div>
              ...
              <p className="text-sm text-yellow-700">
                Manual override will replace the entire invoice total. This should be used with caution.
              </p>
            </div>
          </div>
          ...
          <div className="text-sm leading-6">
            ...
            <p className="font-bold text-red-500">Credit Amount: {(originalTotal - Math.max(0, overrideTotal)).toFixed(2)}</p>
          </div>
        </div>
      )}
      ...
      {step === 2 && creditType === "Flat Value Refund" && (
        <div>
          ...
          <input
            ...
            onChange={(e) => setFlatRefundAmount(Number(e.target.value))}
          />
      ...
      {/* STEP 4: Review (unchanged except for logic caps already handled above) */}
      {step === 4 && (
              ...
              {creditType === "Line-Item Credit"
                ? updatedProducts
                    ...
                    .reduce((sum, a) => {
                      ...
                      const ca = Number(a.creditAmount || 0);
                      return sum + (a.creditType === "percent" ? (total * ca) / 100 : ca);
                    }, 0)
                    .toFixed(2)
      ...
      {step === 7 && (
        <div className="flex flex-col items-center justify-center py-8 text-center">
          ...
          <div className="bg-gray-50 border rounded-lg p-4 w-full sm:w-2/3 text-left text-sm">
            ...
            <p>
              <strong>Action Taken:</strong> {applyTo || "Refund Processed"}
            </p>
            <p>
